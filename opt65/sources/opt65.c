/* Peephole-Optimzier V0.12
   ...
   - 2019/03/06 Laurent Giroud, minor reformat (tabs, layout).
*/
...
#define debug_out stdout // To interlace debug output and the optimized result.
#define dbmsg(par)                                      fprintf(debug_out,par)
#define dbmsg1(par, par1)                               fprintf(debug_out,par, par1)
#define dbmsg2(par, par1, par2)                         fprintf(debug_out,par, par1, par2)
#define dbmsg3(par, par1, par2, par3)                   fprintf(debug_out,par, par1, par2, par3)
#define dbmsg4(par, par1, par2, par3, par4)             fprintf(debug_out,par, par1, par2, par3, par4)
#else
#define dbmsg(par)
#define dbmsg1(par, par1)
#define dbmsg2(par, par1, par2)
#define dbmsg3(par, par1, par2, par3)
#define dbmsg4(par, par1, par2, par3, par4)
#define op2msg(par)
#endif
...
void mapout(int map)
{
  if ((map&reg_a) != 0)  dbmsg("A"); else dbmsg("-");
  if ((map&reg_x) != 0)  dbmsg("X"); else dbmsg("-");
  if ((map&reg_y) != 0)  dbmsg("Y"); else dbmsg("-");
  if ((map&reg_s) != 0)  dbmsg("S"); else dbmsg("-");
  if ((map&mem)   != 0)  dbmsg("M"); else dbmsg("-");
  if ((map&flag_n) != 0) dbmsg("n"); else dbmsg("-");
  if ((map&flag_z) != 0) dbmsg("z"); else dbmsg("-");
  if ((map&flag_c) != 0) dbmsg("c"); else dbmsg("-");
  if ((map&flag_v) != 0) dbmsg("v"); else dbmsg("-");
  if ((map&flag_d) != 0) dbmsg("d"); else dbmsg("-");
  if ((map&flag_i) != 0) dbmsg("i"); else dbmsg("-");
}
...
void lineout(line *p)
{
  if (p->tok == no_tok)
    dbmsg("???");
  else if (p->tok == null_tok)
    dbmsg("---");
  else
    dbmsg1("%s", tok[p->tok].text);
  dbmsg(" dep:");
  mapout(p->dep);
  dbmsg(" mod:");
  mapout(p->mod);
  ...
  dbmsg(" passes:");
  mapout(p->passes);
  dbmsg1(" fl:%2i", p->flags);
  if (p->par != no_par)
  {
    dbmsg2(" \"%s\"  [%i", &parbuf[par_pos[p->par]], p->par);
    if (p->depind & mem)
    {
      dbmsg1("/%i", p->mpar);
      if (p->depind&imem)
...
int opti1()
{
  ...
  while (i < blkbuf_len)
  {
    ...
    if (p->par != no_par && (p->depind&mem) == 0)
    { /* mem=0 so its # or a */
      if (p->depind&absolute)
        val = p->mpar;
      else
...
#ifdef debug
  dbmsg2("erase line %i, replace line %i with...\n", i, j);
  lineout(&blkbuf[j]);
#endif
...
int add_dep(line *p, int hlp, int map)
{
  ...
  if (p->depind & mem)
  {
    ...
    { /* normal addressed memory (direct) */
      if (mpar_flag[p->mpar] != 0 && (p->mod & mem) != 0)
        mpar_flag[p->mpar] &= ~map;
      if (p->dep & mem)
        mpar_flag[p->mpar] |= map;
    }
...
int try_sim2(int i, int j)
{
  ...
  while (x > i)
  {
    ...
    if ((blkbuf[x].depind & mem) != 0 && (blkbuf[x].mod & mem) != 0)
    {
      ...
      else
      {
        ...
        if (mpar_flag[blkbuf[x].mpar] & 1)
          mpar_flag[blkbuf[x].mpar] |= 2;
      }
    }
    x--;
  }
  ...
  /*  1)
  B_mod must not match A_dep  */

  j_end = j + 1;
  while (j_end < blkbuf_len)
  {
    if (blkbuf[j_end].mod & A_dep)
      break;

    if ((blkbuf[j_end].depind & mem) != 0 && (blkbuf[j_end].mod & mem) != 0)
    {
      if ((blkbuf[j_end].depind&(imem | reg_x | reg_y)) != 0 && (A_dep&imem) != 0)
        break;
      else if (mpar_flag[blkbuf[j_end].mpar] & 4)
        break;
    }
    j_end++;
  }
  ...
  while (j_end > j + 1)
  {
    ...
    x = blkbuf_len - 1;
    while (x >= j_end)
      Brest_dep = add_dep(&blkbuf[x--], Brest_dep, 16);
    ...
    x = j_end - 1;
    while (x >= j)
      B_dep = add_dep(&blkbuf[x--], B_dep, 32);
  ...
  while (i < blkbuf_len)
  {
    if (blkbuf[i].tok == as_sta && (blkbuf[i].depind&(reg_x | reg_y)) == 0)
    {
      ...
      while (j < blkbuf_len)
      {
        ...
        {
#ifdef debug
          dbmsg2("### found sta/lda at %i/%i\n", i, j);
#endif
          if (try_sim2(i, j))
            return 1;
        }
        else if (blkbuf[j].par == blkbuf[i].par)
          break;
        j++;
      }
    }
    ...
    if (blkbuf[i].tok == as_tax)
    {
      ...
      while (j < blkbuf_len)
      {
        ...
        {
#ifdef debug
          dbmsg2("### found tax/txa at %i/%i\n", i, j);
#endif
          if (try_sim2(i, j))
            return 1;
        }
        else if (blkbuf[j].dep&reg_x)
          break;
        j++;
      }
    }
...
int com_match2_tax(int j)
{
  ...
  {
    if (!no_dep(com_match_lnum, j, reg_x))
      return 0;
    op2msg("tax,stx -> sta\n");
...
int com_match2_txa(int j)
{
  ...
  {
    if (!no_dep(com_match_lnum, j, reg_a))
      return 0;
    op2msg("txa,sta -> stx\n");
...
int com_match2_tay(int j)
{
  ...
  {
    if (!no_dep(com_match_lnum, j, reg_y))
      return 0;
    op2msg("tay,sty -> sta\n");
    repl1(com_match_lnum, j, j, as_sta);
    return 1;
...
int opti3()
{
  ...
  while (i < blkbuf_len - 1)
  {
    ...
    if (com_match2 == NULL || (blkbuf[i].flags & fixed) != 0)
    {
      i++;
      continue;
    }
    ...
    while (j < blkbuf_len)
    {
      if (blkbuf[j].flags & fixed)
        break; /* don't move through fixed items */
#ifdef debug
      dbmsg2("o+: %i,%i ?\n", i, j);
#endif
      if (com_match2(j))
...
int main(int argc, char **argv)
{
  ...
  while (readline(linebuf, fin) != EOF)
  {
    ...
    if (ret != 0)
    {
      ...
      optimize_block();
      wall_flag = 0;
      if (tmp.tok == no_tok || ret > 1 || (tmp.flags & isjmp) != 0)
      {
        clear_buf();
        puts(linebuf);
      }
